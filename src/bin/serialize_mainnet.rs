...
pub fn main() {
    let setup = Kzg::setup(false).unwrap();
    ...
    fs::write("src/test-files/g1_serialized_mainnet", compressed_g1)
        .expect("Unable to write g1 file");
    fs::write("src/test-files/g2_serialized_mainnet", compressed_g2)
        .expect("Unable to write g2 file");
}
...
mod tests {
    ...
    fn test_deserialize() {
        ...
        let mut compressed_g1 =
            fs::read("src/test-files/g1_serialized_mainnet").expect("Unable to read g1 file");
        let mut compressed_g2 =
            fs::read("src/test-files/g2_serialized_mainnet").expect("Unable to read g2 file");
