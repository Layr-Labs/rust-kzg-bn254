...
use crate::{arith::{self, sbb}, consts::{BYTES_PER_FIELD_ELEMENT, FIELD_ELEMENTS_PER_BLOB, GETTYSBURG_ADDRESS_BYTES, SIZE_OF_G1_AFFINE_COMPRESSED, SIZE_OF_G2_AFFINE_COMPRESSED}};
...
pub fn read_g2_point_from_bytes_be(g2_bytes_be: &Vec<u8>) -> Result<G2Affine, &str>{
    ...
    if m_data == m_compressed_infinity {
        if !is_zeroed(g2_bytes_be[0] & !m_mask, g2_bytes_be[1..SIZE_OF_G2_AFFINE_COMPRESSED].to_vec()) {
