...
impl Blob {
    ...
    pub fn from_bytes_and_pad(input: &[u8]) -> Self {
        ...
        Blob { blob_data: padded_input, is_padded: true, length_after_padding: length_after_padding }
    }
    ...
    pub fn to_polynomial(&self) -> Result<Polynomial, BlobError> {
        if !self.is_padded {
            ...
            let poly = Polynomial::new(&fr_vec, self.length_after_padding).map_err(|err| BlobError::GenericError(err.to_string()))?;
            Ok(poly)
        }
    }
}
...
mod tests {
    ...
    fn test_convert_by_padding_empty_byte(){
        ...
        blob.remove_padding().unwrap();
        assert_eq!(blob.get_blob_data(), vec![104, 105], "testing removing padding");
        ...
        blob.remove_padding().unwrap();
        assert_eq!(blob.is_padded(), false, "cannot be padded");
        assert_eq!(blob.get_blob_data(), GETTYSBURG_ADDRESS_BYTES, "testing removing padding");
