...
/// Copied the referenced bytes array argument into a Vec, inserting an empty byte at the front of every 31 bytes.
/// The empty byte is padded at the low address, because we use big endian to interpret a field element.
/// This ensures every 32 bytes is within the valid range of a field element for the bn254 curve.
/// If the input data is not a multiple of 31 bytes, the remainder is added to the output by
