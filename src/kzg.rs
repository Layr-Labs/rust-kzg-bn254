...
impl Kzg {
    ...
    pub fn parallel_read_g2_points(file_path: String, srs_points_to_load: u32) -> Result<Vec<G2Affine>, KzgError> {

        let (sender, receiver) = bounded::<(Vec<u8>, usize)>(1000);

        // Spawning the reader thread
        let reader_thread = std::thread::spawn(move || -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            Self::read_file_chunks(&file_path, sender, 64, srs_points_to_load).map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { Box::new(e) })
        });
    ...
    pub fn parallel_read_g1_points(file_path: String, srs_points_to_load: u32) -> Result<Vec<G1Affine>, KzgError> {

        let (sender, receiver) = bounded::<(Vec<u8>, usize)>(1000);

        // Spawning the reader thread
        let reader_thread = std::thread::spawn(move || -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            Self::read_file_chunks(&file_path, sender, 32, srs_points_to_load).map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { Box::new(e) })
        });
...
mod tests {
    ...
    fn test_compute_quotient_eval_on_domain(){
        ...
        let result = KZG_3000.compute_quotient_eval_on_domain(z_fr, &eval_fr, value_fr, &roots_of_unities);
