...
mod tests {
    ...
    use rand::Rng;
    use rust_kzg_bn254::{
        blob::Blob, consts::{PRIMITIVE_ROOTS_OF_UNITY, SIZE_OF_G1_AFFINE_COMPRESSED}, errors::KzgError, helpers, kzg::KZG, polynomial::PolynomialCoeffForm
    };
    ...
    #[test]
    fn test_g1_affine_serialze_compressed() {
        let mut rng = rand::thread_rng();
        let point = G1Affine::rand(&mut rng);
        let mut serialized: Vec<u8> = Vec::with_capacity(SIZE_OF_G1_AFFINE_COMPRESSED); 
        point.serialize_compressed(&mut serialized).unwrap();
        let mut helper_serialized: Vec<u8> = Vec::with_capacity(SIZE_OF_G1_AFFINE_COMPRESSED); 
        assert_eq!(serialized.reverse(), helpers::g1_affine_serialze_compressed(&point, &mut helper_serialized).unwrap());
    }
