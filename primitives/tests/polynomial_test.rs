...
mod tests {
    ...
    fn test_is_empty() {
        ...
        assert!(
            !empty_poly_coeff.is_empty(),
            "polynomial coeff form is not empty due to power-of-two padding"
        );
        assert_eq!(
            empty_poly_coeff.len(),
            1,
            "empty input should result in polynomial of length 1"
        );

        let empty_poly_eval = empty_blob.to_polynomial_eval_form().unwrap();
        assert!(
            !empty_poly_eval.is_empty(),
            "polynomial eval form is not empty due to power-of-two padding"
        );
        assert_eq!(
            empty_poly_eval.len(),
            1,
        ...
        let poly_coeff_direct =
            PolynomialCoeffForm::new(vec![Fr::from(1u8), Fr::from(2u8)]).unwrap();
        assert!(
            !poly_coeff_direct.is_empty(),
            "directly created polynomial coeff form should not be empty"
        );

        let poly_eval_direct = PolynomialEvalForm::new(vec![Fr::from(1u8), Fr::from(2u8)]).unwrap();
        assert!(
            !poly_eval_direct.is_empty(),
            "directly created polynomial eval form should not be empty"
        );
        ...
        assert!(
            !poly_coeff_from_empty.is_empty(),
            "polynomial from empty vector is not empty due to padding"
        );
        assert_eq!(
            poly_coeff_from_empty.len(),
            1,
            "empty vector input should result in polynomial of length 1"
        );

        let poly_eval_from_empty = PolynomialEvalForm::new(vec![]).unwrap();
        assert!(
            !poly_eval_from_empty.is_empty(),
            "polynomial from empty vector is not empty due to padding"
        );
        assert_eq!(
            poly_eval_from_empty.len(),
            1,
            "empty vector input should result in polynomial of length 1"
        );
    }
    ...
    fn test_to_bytes_be() {
        ...
        assert_eq!(
            bytes_from_coeff,
            blob.data(),
            "to_bytes_be() should match blob data for coeff form"
        );
        ...
        assert_eq!(
            bytes_from_eval,
            blob.data(),
            "to_bytes_be() should match blob data for eval form"
        );
    }
    ...
    fn test_len_underlying_blob_field_elements() {
        ...
        assert_eq!(
            poly_coeff.len_underlying_blob_field_elements(),
            3,
            "coeff form should have 3 underlying field elements"
        );
        assert_eq!(
            poly_eval.len_underlying_blob_field_elements(),
            3,
            "eval form should have 3 underlying field elements"
        );

        // Verify the relationship: field_elements * BYTES_PER_FIELD_ELEMENT = underlying_blob_bytes
        assert_eq!(
            poly_coeff.len_underlying_blob_field_elements() * 32,
            poly_coeff.len_underlying_blob_bytes(),
            "field elements * 32 should equal blob bytes"
        );
        assert_eq!(
            poly_eval.len_underlying_blob_field_elements() * 32,
            poly_eval.len_underlying_blob_bytes(),
            "field elements * 32 should equal blob bytes"
        );
        ...
        assert_eq!(
            blob_poly_coeff.len_underlying_blob_field_elements(),
            expected_field_elements,
            "blob coeff form should have correct field element count"
        );
        assert_eq!(
            blob_poly_eval.len_underlying_blob_field_elements(),
            expected_field_elements,
            "blob eval form should have correct field element count"
        );

        // Test with large polynomial at size limit
        let max_size_poly_coeff =
            PolynomialCoeffForm::new(vec![Fr::from(1u8); MAINNET_SRS_G1_SIZE]).unwrap();
        assert_eq!(
            max_size_poly_coeff.len_underlying_blob_field_elements(),
            MAINNET_SRS_G1_SIZE,
            "max size polynomial should have correct field element count"
        );
    }
    ...
    fn test_get_at_index() {
        ...
        // Test valid indices
        assert_eq!(
            poly_coeff.get_at_index(0),
            Some(&Fr::from(1u8)),
            "should retrieve first element"
        );
        assert_eq!(
            poly_coeff.get_at_index(1),
            Some(&Fr::from(2u8)),
            "should retrieve second element"
        );
        assert_eq!(
            poly_coeff.get_at_index(2),
            Some(&Fr::from(3u8)),
            "should retrieve third element"
        );
        assert_eq!(
            poly_coeff.get_at_index(3),
            Some(&Fr::from(0u8)),
            "should retrieve padded zero element"
        );

        // Test invalid indices
        assert_eq!(
            poly_coeff.get_at_index(4),
            None,
            "should return None for out of bounds index"
        );
        assert_eq!(
            poly_coeff.get_at_index(100),
            None,
            "should return None for large out of bounds index"
        );

        // Test with empty polynomial
        let empty_poly_coeff = PolynomialCoeffForm::new(vec![]).unwrap();
        assert_eq!(
            empty_poly_coeff.get_at_index(0),
            Some(&Fr::from(0u8)),
            "empty polynomial should have one zero element"
        );
        assert_eq!(
            empty_poly_coeff.get_at_index(1),
            None,
            "should return None for index beyond padded size"
        );
        ...
        assert_eq!(
            large_poly_coeff.get_at_index(127),
            Some(&Fr::from(0u8)),
            "should retrieve zero at end of padded polynomial"
        );
        assert_eq!(
            large_poly_coeff.get_at_index(128),
            None,
            "should return None beyond polynomial length"
        );

        // Verify the polynomial was padded to next power of two (128)
        assert_eq!(
            large_poly_coeff.len(),
            128,
            "large polynomial should be padded to 128 elements"
        );
    }
    ...
    fn test_get_evalualtion() {
        ...
        // Test valid indices
        assert_eq!(
            poly_eval.get_evalualtion(0),
            Some(&Fr::from(5u8)),
            "should retrieve first evaluation"
        );
        assert_eq!(
            poly_eval.get_evalualtion(1),
            Some(&Fr::from(10u8)),
            "should retrieve second evaluation"
        );
        assert_eq!(
            poly_eval.get_evalualtion(2),
            Some(&Fr::from(15u8)),
            "should retrieve third evaluation"
        );
        assert_eq!(
            poly_eval.get_evalualtion(3),
            Some(&Fr::from(0u8)),
            "should retrieve padded zero evaluation"
        );

        // Test invalid indices
        assert_eq!(
            poly_eval.get_evalualtion(4),
            None,
            "should return None for out of bounds index"
        );
        assert_eq!(
            poly_eval.get_evalualtion(100),
            None,
            "should return None for large out of bounds index"
        );
        ...
        assert!(
            blob_poly_eval.get_evalualtion(0).is_some(),
            "should retrieve first evaluation from blob polynomial"
        );
        assert!(
            blob_poly_eval
                .get_evalualtion(blob_poly_eval.len() - 1)
                .is_some(),
            "should retrieve last evaluation"
        );
        assert_eq!(
            blob_poly_eval.get_evalualtion(blob_poly_eval.len()),
            None,
            "should return None beyond length"
        );

        // Test with empty polynomial
        let empty_poly_eval = PolynomialEvalForm::new(vec![]).unwrap();
        assert_eq!(
            empty_poly_eval.get_evalualtion(0),
            Some(&Fr::from(0u8)),
            "empty polynomial should have one zero evaluation"
        );
        assert_eq!(
            empty_poly_eval.get_evalualtion(1),
            None,
            "should return None for index beyond padded size"
        );
    }
    ...
    fn test_point_deserialization_methods() {
        ...
        // Note: BE reader is expected to fail when reading native LE format data
        // due to coordinate interpretation differences, which is the correct behavior
        match be_result {
            Ok(point) => {
