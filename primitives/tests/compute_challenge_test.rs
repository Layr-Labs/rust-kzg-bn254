...
mod tests {
    use rust_kzg_bn254_primitives::{
        ...
        consts::{FIAT_SHAMIR_PROTOCOL_DOMAIN, BYTES_PER_FIELD_ELEMENT, SIZE_OF_G1_AFFINE_COMPRESSED, MAINNET_SRS_G1_SIZE},
    };
    ...
    fn test_compute_challenge_basic_functionality() {
        ...
        assert_ne!(challenge, Fr::from(0u64), "Challenge should not be zero for non-trivial input");
    }
    ...
    fn test_compute_challenge_different_blobs() {
        ...
        assert_ne!(challenge1, challenge2, "Different blob content should produce different challenges");
        assert_ne!(challenge1, challenge3, "Different blob sizes should produce different challenges");
        assert_ne!(challenge2, challenge3, "Different blobs should produce different challenges");
    }
    ...
    fn test_compute_challenge_different_commitments() {
        ...
        assert_ne!(challenge1, challenge2, "Different commitments should produce different challenges");
        assert_ne!(challenge1, challenge3, "Different commitments should produce different challenges");
        assert_ne!(challenge2, challenge3, "Different commitments should produce different challenges");
    }
    ...
    fn test_compute_challenge_empty_blob() {
        ...
        assert!(result.is_ok(), "Empty blob should still produce a valid challenge");
        
        let challenge = result.unwrap();
        // Empty blob should still produce a valid, non-zero challenge due to domain separator and commitment
        assert_ne!(challenge, Fr::from(0u64), "Empty blob challenge should not be zero");
    }
    ...
    fn test_compute_challenge_various_blob_sizes() {
        ...
        for (i, test_data) in test_cases.iter().enumerate() {
            let blob = Blob::from_raw_data(test_data);
            let result = compute_challenge(&blob, &commitment);
            assert!(result.is_ok(), "Challenge computation should succeed for test case {}", i);
        ...
        for i in 0..challenges.len() {
            for j in i+1..challenges.len() {
                assert_ne!(challenges[i], challenges[j], 
                    ...
                    test_cases[i].len(), test_cases[j].len());
            }
        }
    }
    ...
    fn test_compute_challenge_with_identity_point() {
        ...
        assert!(result.is_err(), "Identity point should be rejected as invalid commitment");
        
        // Verify the specific error type
        match result.unwrap_err() {
            rust_kzg_bn254_primitives::errors::KzgError::GenericError(msg) => {
                assert_eq!(msg, "Commitment is not valid", "Should reject identity point with proper error message");
            }
    ...
    fn test_compute_challenge_invalid_commitment_validation() {
        ...
        assert!(identity_point.is_in_correct_subgroup_assuming_on_curve(), "Identity should be in correct subgroup");
        assert_eq!(identity_point, G1Affine::identity(), "Identity point should equal G1Affine::identity()");
    }
    ...
    fn test_compute_challenge_same_content_different_format() {
        ...
        assert_eq!(challenge1, challenge2, "Same data should produce same challenge regardless of construction");
    }
    ...
    fn test_compute_challenge_boundary_conditions() {
        ...
        assert_ne!(challenge32, challenge64, "Different sized blobs should produce different challenges");
        assert_ne!(challenge64, challenge96, "Different sized blobs should produce different challenges");
        assert_ne!(challenge32, challenge96, "Different sized blobs should produce different challenges");
    }
    ...
    fn test_compute_challenge_domain_separation() {
        ...
        assert_eq!(normal_challenge, manual_challenge, 
    ...
    fn test_compute_challenge_large_blob() {
        ...
        assert!(result.is_ok(), "Large blob should not cause compute_challenge to fail");
        
        let challenge = result.unwrap();
        assert_ne!(challenge, Fr::from(0u64), "Large blob challenge should not be zero");
    }
    ...
    fn test_compute_challenge_buffer_size_calculation() {
        ...
        assert!(expected_size < 1_000_000, "Expected size should be reasonable");
    }
    ...
    fn test_compute_challenge_consistent_with_hash_function() {
        ...
        assert_eq!(challenge1, challenge2, "Hash function should be deterministic");
        ...
        assert_ne!(challenge1, different_challenge, 
    ...
    fn test_compute_challenge_oversized_blob_error() {
        ...
        assert!(result.is_err(), "Oversized blob should cause compute_challenge to fail");
        
        // Verify the specific error type
        match result.unwrap_err() {
            rust_kzg_bn254_primitives::errors::KzgError::GenericError(msg) => {
                assert_eq!(msg, "Input size exceeds maximum polynomial size", 
    ...
    fn test_compute_challenge_error_propagation() {
        ...
        match max_result {
            ...
            Err(rust_kzg_bn254_primitives::errors::KzgError::GenericError(msg)) => {
                ...
                assert!(msg.contains("Input size exceeds maximum polynomial size") || 
                       msg.contains("SRS"), "Should fail with size-related error");
            }
            Err(other) => {
    ...
    fn test_compute_challenge_stress_test() {
        ...
        for i in 0..50 {
            ...
            assert_ne!(challenge, Fr::from(0u64), "Challenge {} should not be zero", i);
        }
    }
} 